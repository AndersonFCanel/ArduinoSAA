...
void loop() {
  ...
  if (contaLoop == 3 ) {
    contaLoop = 0;
    sendGetRequest(  "/api/arduino/contaLoop?value=" + String(contaLoopGlobal));
  }
  ...
  if (!n0) {
    if ( n1 || n2 || n3) {
      if ( n1 ) {
        ...
        nomeFalha = String(String("N0_A-N1_L"));
        falhaN1 = true;
      } else if ( n2  ) {
  ...
  if (falhaN0 || falhaN1 || falhaN2 || falhaN3) {
    desligaBomba();
    falha = true;
    sendGetRequest( "/api/arduino/alarme?tipo=" + String(nomeFalha));
  } else {
    falha = false;
  }
  //END-ALARME
